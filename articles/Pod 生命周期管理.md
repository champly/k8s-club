...
`Pod` 是 `Kubernetes` 最小的调度单元，它只是一个逻辑概念，真正运行的是一组共享部分资源的容器集合。一个 `Pod` 经过 `Kube-APIServer` 提交之后，把原数据信息存入到 `ETCD` 中。然后通过 `Kube-scheduler` 进行调度，选择合适的节点运行。到这里 `Pod` 还没有真正运行起来，剩下创建和运行的逻辑全部都在 `Kubelet` 中。接下来我们一起看一下 `Kubelet` 如何对 `Pod` 进行管理的。
...
CSI(Container Storage Interface): 容器存储标准
...
- `Watch`(default): 通过 `watch` 机制获取资源信息，可以动态感知资源变化。
...
- 等待 `VolumeAttachments` 资源变为 `attached`；
